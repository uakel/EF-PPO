...
class Sequential:
    ...
    def step(self, actions):
        ...
        for i in range(len(self.environments)):
            ...
            if "constraint" in env_info:
                constraint.append(env_info["constraint"])
            else:
                constraint.append(-999.0)
        ...
        infos = dict(
            ...
            constraint=np.array(constraint, np.float32),
...
class Parallel:
    ...
    def start(self):
        ...
        self.constraints_list = np.zeros(
            (self.worker_groups, self.workers_per_group), np.float32
        )
    ...
    def step(self, actions):
        ...
        for _ in range(self.worker_groups):
            ...
            self.constraints_list[index] = infos["constraint"]
        ...
        infos = dict(
            ...
            constraint=np.concatenate(self.constraints_list),
